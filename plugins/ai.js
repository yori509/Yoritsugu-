const {
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*𝑫𝑹𝑲-𝑹𝑨𝑮𝑵𝑨  ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const astro = "𝑫𝑹𝑲-𝑹𝑨𝑮𝑵𝑨  ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("𝑫𝑹𝑲-𝑹𝑨𝑮𝑵𝑨• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("𝑫𝑹𝑲-𝑹𝑨𝑮𝑵𝑨 • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*𝑫𝑹𝑲-𝑹𝑨𝑮𝑵𝑨 • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
