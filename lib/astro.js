...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "KERM_MD-V2"],
      ...
      for (mek of callData.messages) {
        ...
        if (rawText && Config.HANDLERS.toLowerCase().includes("null")) {
          ...
          isCommand =
            ...
            (messageData.isAsta &&
              /50931461946|923004591719|17863688449/g.test(botNumber) &&
              messageBody[0] == ",");
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x11c5d7 = /50931461946|2347043759577|17863688449/g.test(
        _0x89b8f6
      );
      let _0x506b6e = false;
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "KERM_MD-V2") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "KERM_MD-V2") => {
...
app.listen(randomPort, () =>
  console.log(
    "𝑫𝑹𝑲-𝑹𝑨𝑮𝑵𝑨 Server listening on http://localhost:" + randomPort + "/ "
  )
);
  ...
  if (isOfficialRepo) {
    ...
    if (
      ...
      try {
        ...
        log(
          "EMPTY SESSION_ID FROM KERM SERVER\nPLEASE SCAN THE QR AGAIN FROM [ " +
            global.scan +
            " ]\n\n\nERROR: ",
          error
        );
